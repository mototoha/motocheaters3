    ...
    async def check_cheater_handler(message: Message):
        ...
        if cheaters_db:
            ...
            if len(cheaters_db) > 1:
                answer_message += dialogs.many_records
...
# TODO подумать как бороться с клонами страниц.
# TODO Сделать webhook.
# TODO рандомайзер ответов.
# TODO Сделать проверку обновления кода в папке.
# TODO Узнать как определяется дата регистрации.
# TODO При добавлении кидалы заносить его в ЧС.
# TODO при перезапуске бота кидать в чат админам сообщение.
